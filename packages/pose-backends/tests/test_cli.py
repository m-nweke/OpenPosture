...
def test_json_always_carries_every_canonical_keypoint(capsys: pytest.CaptureFixture[str]) -> None:
    ...
    A key set that varied with detection would make two captures of the same image diff noisily,
    and would make "this backend stopped reporting a left heel" indistinguishable from "this key
    was never in the format". The table already renders all 34 rows; the machine-readable form is
    the one Epic H compares against the legacy baseline, so it should not be the weaker of the two.
    """
